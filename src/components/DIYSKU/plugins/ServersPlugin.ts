...
class ServersPlugin {
  ...
  private _emitSelectEvent() {
    ...
    // .filter((item: any) => !(item instanceof fabric.GuideLine)) // 过滤掉辅助线
  ...
  renderITextPath(textPaths: Record<'id' | 'path', any>[]) {
    textPaths.forEach((item) => {
      const object = this.canvas.getObjects().find((o: any) => o.id === item.id)
      if (object) {
        fabric.Path.fromObject(item.path, (e: any) => {
          object.set('path', e)
        })
      }
    })
  ...
  async saveJson(json: any) {
    // await transformText(json)
    ...
    this.editor.hooksEntity.hookSaveBefore.callAsync('', () => {
      this.canvas.getObjects().forEach((obj: any) => {
        if (obj instanceof fabric.Image) {
          obj.crossOrigin = 'anonymous' // 设置图像对象的 crossOrigin 属性
        }
      })
  ...
  addImgByElement(target: HTMLImageElement) {
    ...
    if (imgType === 'svg') {
      fabric.loadSVGFromURL(target.src, (objects: any) => {
