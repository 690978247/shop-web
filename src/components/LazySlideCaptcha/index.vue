...
defineOptions({ name: 'LazySlideCaptcha' })
...
const tracks = ref([])
const startSlidingTime = ref<Date | null>()
const showVerifyTip = ref(false)
const showVerifyLoading = ref(false)
const showGenerateLoading = ref(false)
...
const handlerSvgStyle = computed(() => {
  return {
    ...
    height: props.handlerIconHeight + 'px'
  }
})
...
const startRequestGenerate = () => {
  reset()
  showGenerateLoading.value = true
}
// 结束请求生成图片时调用
const endRequestGenerate = (_src: string, _sliderSrc: string) => {
  showGenerateLoading.value = false
  src.value = _src
  sliderSrc.value = _sliderSrc
}
// 开始请求校验时调用
const startRequestVerify = () => {
  showVerifyLoading.value = true
}

// 结束请求校验时调用
const endRequestVerify = (_isPassing: boolean) => {
  isPassing.value = _isPassing
  showVerifyLoading.value = false
...
<template>
  <div class="captcha">
    <div class="captcha__main" :style="imgWrapperStyle">
      ...
      <div class="captcha_message loadding" v-if="showGenerateLoading">
        <div
          class="captcha_message__icon captcha_message__icon--loadding"
        ></div>
        <div class="captcha_message__text">Loading...</div>
      </div>
      <div class="captcha_message" v-if="showVerifyLoading">
        <div
          class="captcha_message__icon captcha_message__icon--loadding"
        ></div>
    ...
    <div class="captcha__bar" :style="dragVerifyStyle" ref="dragVerifyRef">
      <div
    ...
    <div class="captcha__actions" v-if="showRefresh">
      <a class="captcha__action" @click="handleRefresh">
        ...
        <span class="captcha__action__text">Refresh</span>
      </a>
    </div>
  </div>
</template>
